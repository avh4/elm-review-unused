...
"""
        ...
        , test "should report imported type constructors even when they look to be shadowed by a local type alias but they are not exposed in the first place" <|
            \() ->
                [ """module A exposing (x)

import B exposing (Foo(..))

type alias Foo = B.Foo
